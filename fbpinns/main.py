...
def _x_random(subdomain_xs, batch_size, device):
    ...
    s = torch.tensor(
        [[x.min(), x.max()] for x in subdomain_xs], dtype=torch.float32, device=device
    ).T.unsqueeze(
        1
    )  # (2, 1, nd)
    x_random = s[0] + (s[1] - s[0]) * torch.rand(
        (np.prod(batch_size), len(subdomain_xs)), device=device
    )  # random samples in domain
    return x_random
...
def _x_mesh(subdomain_xs, batch_size, device):
    ...
    x_mesh = [
        torch.linspace(x.min(), x.max(), b, device=device)
        for x, b in zip(subdomain_xs, batch_size)
    ]
    x_mesh = torch.stack(torch.meshgrid(*x_mesh), -1).view(
        -1, len(subdomain_xs)
    )  # nb torch.meshgrid uses np indexing="ij"
    return x_mesh
...
def full_model_FBPINN(x, models, c, D):
    ...
    def _single_model(
        im,
    ):  # use separate function to ensure computational graph/memory is released
    ...
    # sum across models
    yj = [
        torch.sum(torch.stack(ts, -1), -1) for ts in zip(*yjs)
    ]  # note zip(*) transposes
...
def full_model_PINN(x, model, c):
    ...
    xmin, xmax = torch.tensor(
        [[x.min(), x.max()] for x in c.SUBDOMAIN_XS],
        dtype=torch.float32,
        device=x.device,
    ).T
    mu = (xmin + xmax) / 2
...
class FBPINNTrainer(_Trainer):
    ...
    def _train_step(
        ...
        for im, i1 in D.active_ims:
            ...
            for iseg in D.m[im]:
                ...
                # sum across models
                yj_seg = [
                    torch.sum(torch.stack(ts, -1), -1) for ts in zip(*yjs_seg)
                ]  # note zip(*) transposes
        ...
        # return result
        return (
            [t.detach() for t in xs],
            [[t.detach() for t in ts] for ts in yjs],
            [[t.detach() for t in ts] for ts in yjs_sum],
            loss.item(),
        )
    ...
    def _test_step(
        ...
        del (
            ...
            yjs_full,
            ys_full_raw,
        )  # fixes weird over-allocation of GPU memory bug caused by plotting (?)
    ...
    def train(self):
        ...
        # create models
        models = [
            c.MODEL(c.P.d[0], c.P.d[1], c.N_HIDDEN, c.N_LAYERS)
            for _ in range(D.N_MODELS)
        ]  # problem-specific

        # create optimisers
        optimizers = [
            torch.optim.Adam(model.parameters(), lr=c.LRATE) for model in models
        ]
        ...
        for i, active in enumerate(A):
            ...
            for im, i1 in D.active_ims:
                mstep += models[im].size  # record number of weights updated
            for im, i1 in D.active_fixed_neighbours_ims:
                fstep += models[im].flops(xs[i1].shape[0])  # record number of FLOPS
            ...
            if (i + 1) % c.SUMMARY_FREQ == 0:
                ...
                # set counters
                rate, gpu_time = c.SUMMARY_FREQ / gpu_time, 0.0

                # print summary
                self._print_summary(i, loss, rate, start)

                # test step
                yj_test_losses = self._test_step(
                    x_test,
                    yj_true,
                    xs,
                    yjs,
            ...
            if (i + 1) % c.MODEL_SAVE_FREQ == 0:
                ...
                np.save(
                    c.MODEL_OUT_DIR + "loss_%.8i.npy" % (i + 1),
                    np.array(yj_test_losses),
                )
...
class FBPINNTrainer_Perm(_Trainer):
    "FBPINN model trainer class"
...
class PINNTrainer(_Trainer):
    ...
    def _train_step(
        self, model, optimizer, c, i, mu, sd, device
    ):  # use separate function to ensure computational graph/memory is released
    ...
    def _test_step(
        self, x_test, yj_true, x, yj, model, c, i, mstep, fstep, writer, yj_test_losses
    ):  # use separate function to ensure computational graph/memory is released
        ...
        if (i + 1) % c.TEST_FREQ == 0:
            ...
            # save figures
            fs = plot_main.plot_PINN(
                x_test, yj_true, x, yj, yj_full, y_full_raw, yj_test_losses, c, i + 1
            )
            if fs is not None:
                self._save_figs(i, fs)
