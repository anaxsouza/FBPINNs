...
import main
import models
from darcy_numeric import darcy_numeric

class Darcy_2D(problems._Problem):

    #    Solves the 2D PDE:
    #     d²u    d²u          
    #    ---- + ---- = (1 - tanh(x)^0.1)(1 - tanh(y)^0.1) - (1 - tanh(1-x)^0.1)(1 - tanh(1-y)^0.1)
    #     dx²    dy²     
    #
    #     u(0,0) =  1
    #     u(1,1) = -1
    ...
    def physics_loss(self, x, y, j0, j1, jj0, jj1):
        ...
        physics = (jj0[:,0] + jj1[:,0]) - ((1 - torch.tanh(x[:,0])**0.1)*(1 - torch.tanh(x[:,1])**0.1) - (1 - torch.tanh(1-x[:,0])**0.1)*(1 - torch.tanh(1-x[:,1])**0.1))
    ...
    def boundary_condition(self, x, y, j0, j1, jj0, jj1, sd):
        ...
        # Apply u = tanh^2((x-0)/sd)*tanh^2((y-0)/sd)*NN + source ansatz
    ...
    def exact_solution(self, x, batch_size):
        ...
        u = darcy_numeric(batch_size_test[0]-1, batch_size_test[1]-1)

        y = torch.tensor(u.flatten(), device=x.device).unsqueeze(1)
...
#subdomain_xs = [np.array([0, .2, .8, 1]), np.array([0, .2, .8, 1])] 
subdomain_xs = [np.arange(0, 1.1, .5), np.arange(0, 1.1, .5)]
